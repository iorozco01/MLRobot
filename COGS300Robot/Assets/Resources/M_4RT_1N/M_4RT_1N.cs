...
public class M_4RT_1N : CogsAgent
{
    ...
    public override void Heuristic(in ActionBuffers actionsOut)
{
        ...
        discreteActionsOut[4] = 0; //Go to base
        discreteActionsOut[5] = 0; //Bully enemy
    ...
    public override void OnActionReceived(ActionBuffers actions){
        ...
        int goToEnemyAxis = (int)actions.DiscreteActions[5];
        
        MovePlayer(forwardAxis, rotateAxis, shootAxis, goToTargetAxis, goToBaseAxis, goToEnemyAxis);


        //Reward agent for bullying the enemy only if it already has one ball in the base, else punish
        if(numInBase() >= 1) {
            if(Vector3.Distance(this.enemy.transform.localPosition, transform.localPosition) < 20) {
                AddReward(1.0f);
            } else {
                AddReward(Vector3.Distance(this.enemy.transform.localPosition, transform.localPosition)*-0.025f);
            }
        } else {
            AddReward(-0.4f);
        }
...
// ----------------------ONTRIGGER AND ONCOLLISION FUNCTIONS------------------------
    ...
    protected override void OnCollisionEnter(Collision collision) 
    {
        ...
        //have a single target in the base
        if (collision.gameObject.CompareTag("Target") && collision.gameObject.GetComponent<Target>().GetInBase() == 1)
        {
            AddReward(1.0f);
        }

        //have a single target carried
        if (collision.gameObject.CompareTag("Target") && collision.gameObject.GetComponent<Target>().GetCarried() == 1)
        {
            AddReward(0.4f);
        }

        //more than one ball being carried or in base
        if(numInBase() > 1 && numCarried() > 1) {
            AddReward(-0.25f);
        }
    ...
    //  --------------------------HELPERS---------------------------- 
     private void AssignBasicRewards() {
        ...
        rewardDict.Add("shooting-laser", 0.3f);
        rewardDict.Add("hit-enemy", 0.7f);
        rewardDict.Add("dropped-one-target", 0f);
        rewardDict.Add("dropped-targets", 0f);
    }
    
    private void MovePlayer(int forwardAxis, int rotateAxis, int shootAxis, int goToTargetAxis, int goToBase, int goToEnemyAxis)
    {
    ...
    //checks to see how many targets are in the base
    private int numInBase() {
        int num = 0;
        foreach (GameObject target in targets){
            if(target.GetComponent<Target>().GetInBase() == 1) {
                num++;
            }
        }
        return num;
    ...
    //checks to see how many targets are carried
    private int numCarried() {
        int num = 0;
        foreach (GameObject target in targets){
            if(target.GetComponent<Target>().GetCarried() >= 1) {
                num++;
            }
        }
        return num;
    ...
    // Go to enemy and bully them by shooting once within range
    private void Bully() {
        float currentDistance = Vector3.Distance(this.enemy.transform.localPosition, transform.localPosition);
        float rotation = GetYAngle(this.enemy);
        TurnAndGo(rotation);
        if(currentDistance < (float)20) {
            SetLaser(true);
        }
    }
